...
class TMCDatabase:
    ...
    def retrieve_from_posts_database(
        ...
        """
        This will change substantially as we update from raw SQL to an ORM.
        """
        #  TODO: rewrite
        keys = kwargs.keys()
        ...
        if 'posted_at_start' in keys:
            ...
            if 'posted_at_end' in keys:
                sql_statement += \
                    f" AND `posted_at` < '{kwargs['posted_at_end']}'"
    ...
    def export_to_csv(self, **kwargs):
        ...
        with open(
            ...
            field_names = [
                'id', 'thread_title', 'username', 'posted_at',
                'message', 'media', 'likes', 'loves',
                'helpful', 'sentiment']
    ...
    def id_gaps_in_scraped_post(self):
        ...
        with self.connection.cursor() as cursor:
            ...
            outstanding_posts = [
                post_id for post_id in list_of_possible_values
                if post_id not in results]
            return outstanding_posts

    def retrieve_posts_for_timeframe(
        self, posted_at_start, posted_at_end, debug=False
    ):
        results = self.retrieve_from_posts_database(
            posted_at_start=posted_at_start,
            posted_at_end=posted_at_end, debug=debug)
    ...
    def graph_amount_of_posts_for_daterange(self, skip=1, **kwargs):
        ...
        datelist = pd.date_range(
            kwargs['start_date'], periods=kwargs['periods']
        ...
        with self.connection.cursor() as cursor:
            for date in datelist:
                sql_statement = (
                    ...
                    "`posted_at` > '{0}' AND `posted_at` < '{1}'".format(
                        date.strftime('%Y-%m-%d 00:00:00'),
                        (date + datetime.timedelta(days=skip)
